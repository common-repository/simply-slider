<?php
/**
 * The SimplySlide Settings Manager Core file
 * 
 ...
 */
if(!function_exists('simplyslide_field_class_loader')):
/**
 * Register a loader to load Field Class dynamically if they exist in fields/ directory
 * 
 * @param string $class name of the class
 */
function simplyslide_field_class_loader($class){
    //let us just get the part after SimplySlide_Settings_Field_ string e.g for SimplySlide_Settings_Field_Text class it loads fields/text.php
    $file_name = strtolower(str_replace( 'SimplySlide_Settings_Field_', '', $class ) );
    //let us reach to the file
    $file = dirname( __FILE__ ). DIRECTORY_SEPARATOR . 'fields'. DIRECTORY_SEPARATOR . $file_name. '.php';
    if( is_readable( $file ) )
        require_once $file; 
}
spl_autoload_register('simplyslide_field_class_loader');
endif;
if(!class_exists('SimplySlide_Settings_Field')):
/**
 ...
 */
class SimplySlide_Settings_Field{
    /**
     *
    ...
    /**
     *
     * @var string Color for the settings field 
     */
    private $color;
    /**
     * 
    ...
    /**
     * Displays a colorpicker field for a settings field
     *
     * @param array   $args settings field args
     */
    function callback_color( $args ) {
        ...
        if('regular' == $size)
            $size= '70px';
         printf( '<input style="width: ' . $size . ';" type="text" name="%2$s" id="%2$s" class="%1$s-text color-picker" value="%3$s" />', $size, $args['option_key'], $value );
         printf( '<span class="description"> %s </span>', $this->get_desc() );  
    }        
    /**
...
if(!class_exists('SimplySlide_Settings_Section')):
class SimplySlide_Settings_Section{
    /**
     *
    ...
    */
     ...
     * 
     * @return SimplySlide_Settings_Section
     */
    public function add_field($field){
        //check if a field class with name SimplySlide_Settings_Field_$type exists, use it 
        $type = 'text';
        if(isset($field['type']))
            $type = $field['type'];//text/radio etc
        $class_name= 'SimplySlide_Settings_Field';
        //a field specific class can be declared as SimplySlide_Settings_Field_typeName
     ...
     * 
     * @see SimplySlide_Settings_Section::add_field()
     * @return SimplySlide_Settings_Section
     * 
     */
     ...
     * @return SimplySlide_Settings_Field
     */
...
if( !class_exists( 'SimplySlide_Settings_Page' ) ):
/**
 ...
 * 
 */
class SimplySlide_Settings_Page {
    /**
     *
     ...
     *
     * @var  SimplySlide_Settings_Section
     */
     ...
     * @return \SimplySlide_Settings_Page
     */
     ...
     * @param  string $title section title
     * @param  string $desc Section description
     * @return return SimplySlide_Settings_Section
     */
    public function add_section( $id, $title, $desc = false ){
        $section_id = $id ;
        $this->sections[$section_id] = new SimplySlide_Settings_Section( $id, $title, $desc );        
...
if( !class_exists('SimplySlide_Settings_Manager') ):
/**
 * Settings Page store
 ...
 * 
 */
class SimplySlide_Settings_Manager{
    /**
     *
    ...
    /**
     * 
     * @return SimplySlide_Settings_Manager
     */
     ...
     * @param SimplySlide_Settings_Page $page
     * @return SimplySlide_Settings_Page
     */
    function add_page( $page_name, $page=false ){
        if( !$page )
            $page = new SimplySlide_Settings_Page( $page_name );
    ...
    public function load_js() {
       wp_enqueue_media();
       wp_enqueue_script('simplyslider-media-uploader', $this->url.'_inc/uploader.js', array('jquery'));
       ...
       wp_enqueue_script('simplyslider_colorpicker', $this->url.'_inc/colorpicker.js', array('jquery'));
    }
}
SimplySlide_Settings_Manager::get_instance();
endif;